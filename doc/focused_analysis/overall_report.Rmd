---
title: "#zerowaste subset - overall report"
output:
  github_document
---
...
This notebook aims to explore the pre-agreed subset of the #zerowaste tweets concerning the analysis's original dimensions. We focus on the distinct tweets written in English, with locations pinned down to the US or UK. The document is structured as follows:

1 [Housekeepin']  
2 [Subsetting]  
3 [Exploratory data analysis]  
&nbsp;&nbsp;3.1 [Computing characteristics]  
&nbsp;&nbsp;3.2 [Visualizations]  
4 [Next steps]
...
```{r setup, include=F}
to_install = c("data.table", "bit64", "tidyverse", "janitor", "devtools", "stringdist")
...
We start with checking the essential properties of the subset. The data consists of `r nrow(raw_tweets)` rows and `r ncol(raw_tweets)` columns. Furthermore, we see `r nu_users` unique user accounts; however, only `r nu_verified` verified. We observe `r nid_tweets` of unique tweets based on `id`, and `r nu_tweets` distinct tweets are based on the text itself. Similarly, we identify `r nid_retweets` retweets based on reference, and `r nu_retweets` based on naive RT detection. This is to be expected.
...
From the left plot, we can see that approx. half of the tweets are shorter than 140 chars; however, some extended texts are almost 1000 char long. The middle plot shows that 75 % of the tweets consist of 29 words or less. Similarly, the right chart displays that 50 % of the tweets employ less than four hashtags. There appears to be a slight improvement of the token & tag count compared to the original set.
...
On the left, we see that even though most popular tweet sources are organic, there is a considerable amount of marketing automation apps. On the right, we see approximate locations of the tweets on a state level.

## Next steps

We eliminated most of the issues from the preliminary part of the research with straightforward refinement. However, the resulting subset needs to further analyzed concerning the textual content & topic models.
